...
This block maintains [decision-matrix.md](decision-matrix.md): effective doubts indexed by **this block's elements only** (UC IDs).
...
| `decision-matrix.md` | Effective doubt per `(element, event)` — local `D-XXX` or cross-block `[block/D-XXX](…/solved/…)` |
