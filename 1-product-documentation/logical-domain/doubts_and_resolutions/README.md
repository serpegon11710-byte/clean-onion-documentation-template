...
Manages questions and decisions for `logical-domain/`.
...
This block maintains [decision-matrix.md](decision-matrix.md): effective doubts indexed by **this block's elements only** (entity IDs).
...
| `decision-matrix.md` | Effective doubt per entity `(element, event)` — local `D-XXX` or cross-block `[block/D-XXX](…/solved/…)` |
